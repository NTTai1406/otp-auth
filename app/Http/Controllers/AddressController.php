...
class AddressController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Lấy danh sách thành phố, huyện, xã, và đường phố dựa trên các lựa chọn
        $cities = $selectedCountry ? City::where('country_id', $selectedCountry)->get() : [];
        $districts = $selectedCity ? District::where('city_id', $selectedCity)->get() : []; // Sửa ở đây
        $wards = $selectedDistrict ? Ward::where('district_id', $selectedDistrict)->get() : []; // Sửa ở đây
        $streets = $selectedWard ? Street::where('ward_id', $selectedWard)->get() : []; // Sửa ở đây
        ...
        if ($selectedCountry && $selectedCity && $selectedDistrict && $selectedWard && $selectedStreet) {
            ...
            if ($country && $city && $district && $ward && $street) {
    ...
    public function storeAddress(Request $request)
    {
        ...
        return redirect('/index')->with('success', 'Address has been added successfully');
    }
}
